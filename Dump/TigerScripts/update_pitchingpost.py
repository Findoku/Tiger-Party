...
csv_file_name = 'pitchingpost.csv'
...
table_columns = [
    'playerID', 'yearID', 'round', 'teamID', 'p_W', 'p_L', 'p_G', 'p_GS', 
        ...
        try:
            ...
            for csv_col, db_col in column_mapping.items():
                ...
                if value == '':  # Handle empty strings as default
                    ...
                    cleaned_row[db_col] = 0 if db_col.startswith('p_') or db_col in ['yearID'] else None
                elif db_col in ['p_BAOpp', 'p_ERA']:  # Handle decimal fields
                    cleaned_row[db_col] = float(value) if value else 0.0
                elif db_col.startswith('p_') or db_col in ['yearID']:  # Handle integer fields
                    cleaned_row[db_col] = int(value) if value.isdigit() else 0
            ...
            query = f"""
            INSERT INTO pitchingpost ({', '.join(table_columns)}) 
            VALUES ({placeholders})
...
print(f"Data from {csv_file_path} successfully imported into the 'pitchingpost' table.")
